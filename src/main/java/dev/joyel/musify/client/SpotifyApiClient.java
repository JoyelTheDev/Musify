...
public class SpotifyApiClient {
   ...
   private SpotifyApiClient() {
      this.errorHandler.setStatusListener((status) -> {
         switch (status) {
            ...
            case ONLINE -> this.notifyStatusMessage(null);
   ...
   private String getClientId() {
      ...
      return customId != null && !customId.isEmpty() ? customId : DEFAULT_CLIENT_ID;
   }

   private String getClientSecret() {
      String customSecret = this.config.getSpotifyClientSecret();
      return customSecret != null && !customSecret.isEmpty() ? customSecret : DEFAULT_CLIENT_SECRET;
   }
   ...
   public TrackInfo getCurrentTrack() {
      if (this.errorHandler.isOffline() && this.currentTrack == null && this.lastKnownTrack != null) {
         return new TrackInfo(this.lastKnownTrack.getSpotifyId(), this.lastKnownTrack.getTitle(), 
            this.lastKnownTrack.getArtist(), this.lastKnownTrack.getProgressMs(), 
            this.lastKnownTrack.getDurationMs(), false, this.lastKnownTrack.getAlbumArtUrl());
      }
   ...
   public void playPauseAsync() {
      ExecutorManager.getInstance().submit(() -> {
         if (this.errorHandler.isOffline()) {
            ...
            ErrorHandler.Result<Void> result = this.errorHandler.executeWithRetry("spotify/play-pause", () -> {
               if (!this.ensureAuthorized()) {
                  return ErrorHandler.Result.failure(ErrorHandler.ErrorCategory.AUTH_EXPIRED);
               }
               
               TrackInfo track = this.getCurrentTrack();
               String endpoint = track != null && track.isPlaying() 
                  ? "https://api.spotify.com/v1/me/player/pause" 
                  : "https://api.spotify.com/v1/me/player/play";
               
               HttpRequest req = HttpRequest.newBuilder()
                  .uri(URI.create(endpoint))
                  .header("Authorization", "Bearer " + this.config.getAccessToken().orElse(""))
                  .PUT(BodyPublishers.noBody())
                  .timeout(Duration.ofSeconds(10L))
                  .build();
               
               HttpResponse<String> resp = this.httpClient.send(req, BodyHandlers.ofString());
               ErrorHandler.ErrorCategory error = this.errorHandler.categorizeHttpCode(resp.statusCode(), resp.body());
               
               if (error != null) {
                  return ErrorHandler.Result.failure(error);
               }
               return ErrorHandler.Result.success(null);
            });
   ...
   public void nextTrackAsync() {
      ExecutorManager.getInstance().submit(() -> {
         if (this.errorHandler.isOffline()) {
            ...
            ErrorHandler.Result<Void> result = this.errorHandler.executeWithRetry("spotify/next", () -> {
               if (!this.ensureAuthorized()) {
                  return ErrorHandler.Result.failure(ErrorHandler.ErrorCategory.AUTH_EXPIRED);
               }
   ...
   public void previousTrackAsync() {
      ExecutorManager.getInstance().submit(() -> {
         if (this.errorHandler.isOffline()) {
            ...
            ErrorHandler.Result<Void> result = this.errorHandler.executeWithRetry("spotify/prev", () -> {
               if (!this.ensureAuthorized()) {
                  return ErrorHandler.Result.failure(ErrorHandler.ErrorCategory.AUTH_EXPIRED);
               }
   ...
   private void handlePollError(Exception e) {
      ...
      if (failures >= MAX_POLLING_FAILURES_BEFORE_SLOWDOWN) {
   ...
   private void pollPlaybackWithRetry() {
      if (!this.errorHandler.isCircuitOpen("spotify/playback")) {
         ErrorHandler.Result<TrackInfo> result = this.errorHandler.executeWithRetry("spotify/playback", () -> {
            try {
   ...
   private ErrorHandler.Result<TrackInfo> pollPlaybackInternal() throws IOException, InterruptedException {
      ...
      if (resp.statusCode() == 200) {
         ...
         return ErrorHandler.Result.success(track);
      } else if (resp.statusCode() == 204) {
         if (this.currentTrack != null) {
            TrackInfo pausedTrack = new TrackInfo(this.currentTrack.getSpotifyId(), this.currentTrack.getTitle(), 
               this.currentTrack.getArtist(), this.currentTrack.getProgressMs(), 
               this.currentTrack.getDurationMs(), false, this.currentTrack.getAlbumArtUrl());
            return ErrorHandler.Result.success(pausedTrack);
         } else {
            return ErrorHandler.Result.success(null);
         }
         ...
         return ErrorHandler.Result.failure(ErrorHandler.ErrorCategory.RATE_LIMITED, "Rate limited. Waiting " + waitMs / 1000L + "s...");
      } else {
         ErrorHandler.ErrorCategory error = this.errorHandler.categorizeHttpCode(resp.statusCode(), resp.body());
         return ErrorHandler.Result.failure(error != null ? error : ErrorHandler.ErrorCategory.UNKNOWN);
      }
   }
   ...
   public synchronized void startAuthorization() {
      ...
      String url = "https://accounts.spotify.com/authorize?response_type=code&client_id=" + 
         urlEncode(this.getClientId()) + "&scope=" + urlEncode(SCOPES) + "&redirect_uri=" + urlEncode(REDIRECT_URI);
   ...
   private void exchangeCodeForTokens(String code) {
      try {
         ...
         String body = "grant_type=authorization_code&code=" + urlEncode(code) + "&redirect_uri=" + urlEncode(REDIRECT_URI);
         HttpRequest req = HttpRequest.newBuilder()
   ...
   public void refreshAccessTokenAsync() {
      ExecutorManager.getInstance().submit(() -> {
         try {
            ...
            String auth = Base64.getEncoder().encodeToString((this.getClientId() + ":" + this.getClientSecret()).getBytes(StandardCharsets.UTF_8));
            String body = "grant_type=refresh_token&refresh_token=" + urlEncode(refreshOpt.get());
            HttpRequest req = HttpRequest.newBuilder()
               .uri(URI.create("https://accounts.spotify.com/api/token"))
               .header("Authorization", "Basic " + auth)
               .header("Content-Type", "application/x-www-form-urlencoded")
            ...
            if (resp.statusCode() == 200) {
               ...
               String access = json.has("access_token") ? json.get("access_token").getAsString() : null;
               int expiresIn = json.has("expires_in") ? json.get("expires_in").getAsInt() : 3600;
               String newRefresh = json.has("refresh_token") ? json.get("refresh_token").getAsString() : refreshOpt.get();
               if (access != null) {
   ...
   private class OAuthCallbackHandler implements HttpHandler {
      ...
      public void handle(HttpExchange exchange) throws IOException {
         ...
         if (query != null && query.contains("code=")) {
            ...
            ExecutorManager.getInstance().submit(() -> SpotifyApiClient.this.exchangeCodeForTokens(finalCode));
         } else {
